/**
 * Reusable demo-data generator (no Telegram, no top-level side effects).
 *
 * Fabricates a realistic-looking dataset so the dashboard is understandable at
 ...
 *
 * Curator ids are deterministic (900000+i) and cards upsert by key, so this is
 * idempotent — re-running updates rows instead of multiplying them. Used by:
 *   • `src/dev/simulate.ts` (the local dev/demo harness), and
 *   • `src/index.ts` on an empty DB in web-only (no-BOT_TOKEN) mode, so a
 *     sandbox deploy (e.g. PLN LabOS, which injects no secrets) shows a live
 *     dashboard instead of an empty shell.
 */
...
function runVotes(curatorId: number, role: string, cards: Card[], n: number) {
...
/**
 * Populate the DB with sample cards, ~40 curators, and votes across the current
 * and previous editions. Idempotent (see module doc). Returns a small summary.
 */
export function seedDemoData(): { curators: number; votes: number } {
  ...
  const pastCards = repo.getAllCards().filter((c) => c.edition === prevEdition())
  ...
  NAMES.forEach((name, i) => {
    ...
    runVotes(id, role, currentCards, 6 + Math.floor(Math.random() * 14))
    if (Math.random() < 0.7 && pastCards.length >= 2) {
      runVotes(id, role, pastCards, 4 + Math.floor(Math.random() * 8))
    }
  })

  const top = globalLeaderboard()
    .filter((c) => currentCards.some((cc) => cc.id === c.id))
    .slice(0, 3)
    .map((c) => c.title)
  console.log('[demo] seeded', NAMES.length, 'curators. Current top:', top)
  return { curators: NAMES.length, votes: repo.totalVotes() }
}
